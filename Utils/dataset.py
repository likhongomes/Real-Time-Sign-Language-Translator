...
def asl_collate_fn(batch):
    """
    Collate function for variable-length ASL sequences.
    Returns:
        landmarks: (B, T, F)
        labels: (B,)
        seq_lens: (B,)
...
class ASLDataset(Dataset):
    ...
    def __init__(
        ...
        # Setup caching first so _load_annotations can see cache state
        self.use_cache = use_cache
        self.cache_dir = cache_dir
        self.cache = FeatureCache(cache_dir)

        # Load annotations (uses cache presence instead of video existence only)
    ...
    def _load_annotations(self, json_path: str) -> List[Dict]:
        ...
        In cache-based mode, videos may have been deleted after
        feature extraction. Accept a sample if either:
        - the video still exists, or
        - cached landmarks exist for its path.
        """
        samples: List[Dict] = []
        ...
        if os.path.exists(json_path):
            ...
            for i, ann in enumerate(annotations):
                ...
                has_cache = self.cache.exists(video_str) if self.use_cache else False

                if has_video or has_cache:
                    samples.append(
                        {
            ...
            for label_dir in self.data_dir.iterdir():
                if label_dir.is_dir():
                    ...
                    for video_file in label_dir.glob("*.mp4"):
                        ...
                        has_video = os.path.exists(video_str)
                        has_cache = self.cache.exists(video_str) if self.use_cache else False
                        if has_video or has_cache:
                            samples.append(
                                {
...
class ASLFingerspellingDataset(ASLDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.samples = [s for s in self.samples if s["label"].upper() in self.LETTERS]
...
if __name__ == "__main__":
    ...
    if data_path.exists():
        ...
        if len(dataset) > 0:
            ...
            loader = DataLoader(dataset, batch_size=4, shuffle=True, collate_fn=asl_collate_fn)
